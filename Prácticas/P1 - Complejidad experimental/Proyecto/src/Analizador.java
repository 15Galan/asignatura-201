...
public class Analizador {
	...
	private static final String[] complejidades = {"1", "NLOGN", "N", "LOGN", "N2", "N3", "2N", "NF"};
	private static final double TIEMPO_MAX = 10 * Math.pow(10, 9);	// Tiempo de ejecución que no se debe sobrepasar (nseg)
...
//		System.out.println("n <- " + Arrays.toString(n) + "\n");
		...
		for (String complejidad : complejidades) {
			Datos valor = new Datos(complejidad, adaptarMedias(complejidad, tiempos));

			tabla.put(complejidad, valor);
		}
...
//		System.out.println("\n- FINAL -\n\n" + String.format("%.3f Seg", (double) ejecucion.tiempoPasado()*Math.pow(10, -9)));
	...
	/**
	 * Ejecuta el algoritmo varias veces con distintos valores y almacena su
	 * media de tiempos en una lista; esto solo se llevará a cabo hasta
	 * que el tiempo de ejecución supere el tiempo límite indicado
	 *
	 * @param intentos Veces que el algoritmo se ejecutará para un mismo valor
	 * @param tiempoMax	Duración aproximada (nseg) de este método (debe ser bastante menor que TIEMPO_MAX)
	 *
	...
	private static List<Double> calcularTiempos(int intentos, double tiempoMax) {
		...
		while (i < n.length && continuar) {
			...
			while (j < intentos && continuar) {
				duracion += intentar(n[i]);			// Medir el tiempo de ejecución del algoritmo
...
//				System.out.println("\tT(" + n[i] + ") = " + duracion/intentos * Math.pow(10, -9) + " Seg");
	...
	/**
	 * Realiza una ejecución del algoritmo para un determinado valor
	 *
	 * @param n Valor de entrada del algoritmo
	 *
	 * @return El tiempo que tardó el algoritmo en procesar el valor anterior
	 */
	private static long intentar(long n) {
		Temporizador temporizador = new Temporizador(2);	// Trabajará en modo 2: NANOsegundos
	 ...
	 * complejidad: f(1), f(log(n)), f(n), f(n·log(n))... y la almacena en una lista
	 *
	 * @param complejidad Complejidad para la que se quiere adaptar las medias
	 * @param medias Lista de tiempos medios de ejecución de un algoritmo (nseg)
	 *
	...
	/**
	 * Dado un mapa cuyas claves son complejidades y sus valores asociados son
	 * objetos de la clase Datos (que contiene el nombre de la complejidad, sus
	 * tiempos medios adaptados y su ratio), comprueba cuál de las complejidades
